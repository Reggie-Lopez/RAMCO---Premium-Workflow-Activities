...
namespace Remotion.Linq.Clauses.ResultOperators
{
  ...
  public sealed class TakeResultOperator : SequenceTypePreservingResultOperatorBase
  {
    ...
    public override ResultOperatorBase Clone(CloneContext cloneContext) => (ResultOperatorBase) new TakeResultOperator(this.Count);

    public override StreamedSequence ExecuteInMemory<T>(StreamedSequence input) => new StreamedSequence((IEnumerable) input.GetTypedSequence<T>().Take<T>(this.GetConstantCount()).AsQueryable<T>(), this.GetOutputDataInfo(input.DataInfo));
    ...
    public override string ToString() => "Take(" + this.Count.BuildString() + ")";
  }
}
