...
namespace Remotion.Linq.Clauses.ResultOperators
{
  ...
  public sealed class IntersectResultOperator : SequenceTypePreservingResultOperatorBase
  {
    ...
    public override ResultOperatorBase Clone(CloneContext cloneContext) => (ResultOperatorBase) new IntersectResultOperator(this.Source2);

    public override StreamedSequence ExecuteInMemory<T>(StreamedSequence input)
    {
      ArgumentUtility.CheckNotNull<StreamedSequence>(nameof (input), input);
      return new StreamedSequence((IEnumerable) input.GetTypedSequence<T>().Intersect<T>(this.GetConstantSource2<T>()).AsQueryable<T>(), this.GetOutputDataInfo(input.DataInfo));
    }
    ...
    public override string ToString() => "Intersect(" + this.Source2.BuildString() + ")";
  }
}
