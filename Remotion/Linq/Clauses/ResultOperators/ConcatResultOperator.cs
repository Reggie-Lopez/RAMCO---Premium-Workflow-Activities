...
// Type: Remotion.Linq.Clauses.ResultOperators.ConcatResultOperator
...
namespace Remotion.Linq.Clauses.ResultOperators
{
  ...
  public sealed class ConcatResultOperator : SequenceFromSequenceResultOperatorBase, IQuerySource
  {
    ...
    public override ResultOperatorBase Clone(CloneContext cloneContext) => (ResultOperatorBase) new ConcatResultOperator(this._itemName, this._itemType, this._source2);

    public override StreamedSequence ExecuteInMemory<T>(StreamedSequence input) => new StreamedSequence((IEnumerable) input.GetTypedSequence<T>().Concat<T>((IEnumerable<T>) this.GetConstantSource2()).AsQueryable<T>(), (StreamedSequenceInfo) this.GetOutputDataInfo((IStreamedDataInfo) input.DataInfo));
    ...
    public override string ToString() => "Concat(" + this.Source2.BuildString() + ")";
  }
}
